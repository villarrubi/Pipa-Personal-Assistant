...
static esp_err_t panel_st77916_disp_off(esp_lcd_panel_t *panel, bool off);
#endif
...
#else
    st77916->base.disp_off = panel_st77916_disp_off;
#endif
...
static const st77916_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // {0xE1, (uint8_t []){0xF0, 0x0D, 0x15, 0x0A, 0x0A, 0x26, 0x3B, 0x43, 0x50, 0x37, 0x1…401 tokens truncated…int8_t []){0x42}, 1, 0},
    // {0x71, (uint8_t []){0xE0}, 1, 0},
    // {0x72, (uint8_t []){0x40}, 1, 0},
    // {0x73, (uint8_t []){0x40}, 1, 0},
    // {0x74, (uint8_t []){0x02}, 1, 0},
...
static esp_err_t panel_st77916_disp_off(esp_lcd_panel_t *panel, bool off)
{
    return panel_st77916_disp_on_off(panel, !off);
}
#endif
