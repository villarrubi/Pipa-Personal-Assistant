...
$requiredPatterns = @(
    ...
    @($commandEditor, 'placeholders', 'rendered(with values:', 'toolArguments(with values:', 'isSafeMessageText', '4000', 'Control', 'privacySensitive', 'Preparar en el editor', 'Enviar acción estructurada', 'without executing it'),
    ...
    @($coreTests, 'testRecordLayerEncryptsAuthenticatesAndRejectsReplay', 'testRecordLayerMatchesTheSharedPythonVector', 'testTCPClientRejectsInvalidTextAndOversizedArgumentsBeforeTransport', 'testMobileTextPolicyRejectsProtocolAndBidirectionalControls', 'testMobileSettingsRejectUnsafePersistedValuesBeforeTransport'),
