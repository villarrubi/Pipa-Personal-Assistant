...
$requiredPatterns = @(
    ...
    @($protocol, 'Curve25519.Signing', 'ChaChaPoly', 'HKDF<SHA256>.deriveKey', 'sharedSecretData', 'maxFrameBytes', 'publicKeyDigest(forPublicKeyData', 'encodeBase64URL(data) == value', 'sequence < UInt64.max', 'closed = true', 'guard Set(frame.keys) == Self.recordFields', 'PipaMobileTextPolicy', 'containsDisplayControl', 'isSafeMessageText', 'PipaMobileRequestBinding', 'digest(forToolName'),
    ...
    @($tcp, 'NWConnection', 'serverPublicKeyData', 'device_hello', 'catalog_request', 'PipaMobileCatalog', 'requestCatalogDetails', 'commands.count <= 64', 'parseCapabilities', 'validateServerMessage', 'serverFieldContract', 'serverRequiredFields', 'requiredFields.isSubset', 'capabilityGroups', 'booleanCapabilityFields', 'contact_aliases_configured', 'isCapabilityValue', 'isAllowedHost', 'first != "0"', 'first == 192', 'containsProtocolControl', 'firmwareVersion == firmwareVersion.trimmingCharacters', 'PipaMobileTextPolicy.isSafeDisplayText(firmwareVersion', 'PipaMobileTextPolicy.isSafeDisplayText($0', 'maxArgumentsBytes', 'PipaMobileRequestBinding', 'request_digest', 'requestInFlight', 'requestInProgress', 'asyncAfter', 'connectionTimeout', 'ioTimeout', 'receiveBuffer'),
    @($viewModel, 'PipaMobileTCPClient', 'PipaKeychainIdentityStore', 'PipaMobileSettingsStoring', 'PipaMobileSettingsStore', 'PipaMobileIntegration', 'PipaMobileCommandParameter', 'structuredParameterKinds', 'guild_id', 'parameters', 'integrationCapabilities', 'requestCatalogDetails', 'pendingConfirmation', 'PipaMobileConfirmation', 'expiresAt', 'isExpired', 'localPreview', 'localPreviewToolName', 'localRequestDigest', 'serverRequestDigest', 'localPreviewMatchesServerAction', 'safeLocalPreview', 'clearLocalPreview', 'resolveConfirmation', 'La confirmación ha caducado; vuelve a conectar.', 'prepareIdentity', 'identityFingerprint', 'serverFingerprint', 'serverFingerprintVerified', 'markServerFingerprintVerified', 'invalidateServerFingerprintVerification', 'useCommand', 'useCommandText', 'updateVoiceDraft', 'forgetConnectionSettings', 'sendStructuredCommand', 'PipaMobileRequestBinding.digest', 'PipaMobileRequestBinding.isValidDigest', 'expires_at', 'expiresAt > 0', 'callID', 'operationTask', 'requestTask', 'requestInProgress', 'sessionGeneration', 'connectInProgress', 'Task.isCancelled', 'await newClient.disconnect()', 'closeAfterOperationFailure', 'requiresConfirmation == (safety == "unsafe")', 'parseCatalogCommands', 'parseIntegrationCapabilities', 'Set(parsed.map(\.id)).count == parsed.count', 'isSafeConfirmationSummary', 'guard let expected = deviceConfirmationSummaries[toolName] else', 'whatsapp_phone_open', 'La respuesta del agente no es válida.'),
    @($commandEditor, 'placeholders', 'catalogPlaceholders(in:', 'rendered(with values:', 'toolArguments(with values:', 'PipaMobileDestinationPolicy', 'normalizePhone', 'normalizeSnowflake', 'isSafeMessageText', '4000', 'Control', 'privacySensitive', 'Preparar en el editor', 'Enviar acción estructurada', 'without executing it'),
    @($appleMusic, 'MusicCatalogSearchRequest', 'MusicAuthorization', 'SystemMusicPlayer', 'search(term:', 'operationGeneration', 'guard self.operationGeneration == generation', 'beginAsyncOperation()', 'finishAsyncOperation', 'play(result:', 'skipToNextEntry', 'skipToPreviousEntry', 'player.stop()', 'previousTrack()', 'stopPlayback()', 'clearEphemeralState()', 'PipaMobileTextPolicy'),
    ...
    @($wakeOnLanController, 'PipaMobileWakeOnLanController', 'NWConnection', 'NWParameters.udp', '255.255.255.255', 'maximumDatagramSize', 'requestInProgress', 'DispatchQueue.main.async', 'magicPacket', 'validate(mac:', 'cancel()', '5'),
    ...
    @($view, 'PipaMobileRootView', 'LocalIntegrationAction', 'Equatable', 'pendingLocalIntegrationAction', '.confirmationDialog', 'performPendingLocalIntegrationAction', 'clearLocalIntegrationInputs', 'localWhatsAppPhone = ""', 'localWhatsAppMessage = ""', 'localDiscordChannelID = ""', 'localDiscordGuildID = ""', 'scenePhase', 'onChange', 'commandToEdit', '.sheet(item:', 'command.placeholders', 'integrationSection', 'integrationCapabilities', 'localWebSearchSection', 'Internet en este iPhone', 'localWakeOnLanSection', 'PipaMobileWakeOnLan', 'Encender PC', 'localAppleMusic', 'Apple Music en este iPhone', 'localMessagingSection', 'WhatsApp y Discord en este iPhone', 'whatsappComposeURL', 'discordChannelURL', 'Anterior', 'Detener', 'privacySensitive', 'Disponible', 'No disponible', 'Preparar identidad', 'Fingerprint:', 'Fingerprint del agente:', 'He comparado el fingerprint', 'Fingerprint verificado', 'Confirmar acción', 'Comando preparado en este iPhone', 'localPreviewMatchesServerAction', 'Rechazar', 'Aceptar', 'pending.isExpired', '.disabled(pending.isExpired || !pending.localPreviewMatchesServerAction)', 'Usar', 'PipaMobileSpeechRecognizer', 'Dictar comando', 'Parar dictado', 'updateVoiceDraft', 'speechRecognizer.cancel()', 'speechRecognizer.isListening', 'Borrar configuración guardada', 'case webSearch(query: String)', 'case wakeOnLan(mac: String)', 'case whatsappMessage(phone: String, message: String)', 'case discordCall(channelID: String, guildID: String?)', 'exact validated values', 'pendingLocalIntegrationAction = nil', 'pendingLocalIntegrationAction = .webSearch(query:', 'pendingLocalIntegrationAction = .wakeOnLan(mac:', 'case let .webSearch(query)', 'case let .whatsappMessage(phone, message)'),
    ...
    @($coreTests, 'testRecordLayerEncryptsAuthenticatesAndRejectsReplay', 'testBinaryRecordRejectsUnknownFieldsAndClosesTheLayer', 'testRecordLayerMatchesTheSharedPythonVector', 'testStructuredRequestBindingMatchesThePythonCanonicalVector', 'testServerEnvelopeRejectsUnknownFieldsWrongVersionsAndMissingRequiredFields', 'testTCPClientRejectsInvalidTextAndOversizedArgumentsBeforeTransport', 'testCatalogCapabilitiesAcceptTheRealIntegrationMatrix', 'testMobileEndpointRejectsPublicAndWildcardHosts', '192.168.001.020', 'testMobileTextPolicyRejectsProtocolAndBidirectionalControls', 'testDestinationPolicyUsesCanonicalPhoneAndDiscordIDs', 'testMobileSettingsRejectUnsafePersistedValuesBeforeTransport'),
    ...
    @($uiTests, 'testConnectionRequiresEphemeralFingerprintAcknowledgement', 'testSavedSettingsNeverCountAsFingerprintAcknowledgement', 'testCatalogCommandEditorRendersBoundedArgumentsWithoutSending', 'testCatalogRejectsDuplicateAndOversizedCommandLists', 'testCatalogRejectsPlaceholderParameterDrift', 'testStructuredCatalogCommandAcceptsMessageLineFeedsAndTypedArguments', 'testStructuredCatalogCommandRejectsMalformedParameterMetadata', 'testVoiceDraftOnlyUpdatesTheEditorWithoutSending', 'testVoiceDraftRejectsControlCharactersAndOversizedInput', 'testDisconnectClearsThePrivateCommandDraft', 'testForgetConnectionSettingsClearsDraftWhenAlreadyDisconnected', 'testCatalogRejectsBidirectionalFormattingControls', 'testIntegrationCapabilitiesShowOnlyCoarseManualActionStatus', 'testIntegrationCapabilitiesRejectMalformedGroupsAtomically', 'testLocalAppleMusicControllerStartsWithoutAuthorizationOrTransport', 'testLocalWakeOnLanStartsIdleAndDoesNotRequireAgentTransport', 'testLocalWebSearchLinkIsFixedAndEncodesTheQuery', 'testLocalWebSearchLinkRejectsEmptyUnsafeOrOversizedQueries', 'testLocalWhatsAppLinkPreparesMessageWithoutChangingItsContents', 'testLocalWhatsAppLinkRejectsInvalidOrUnsafeInput', 'testLocalDiscordLinkUsesDMByDefaultAndServerWhenProvided', 'testLocalDiscordLinkRejectsInvalidIdentifiers', 'testConfirmationSummaryRejectsArgumentsAndAllowsFixedLabels', 'testLocalConfirmationPreviewIsBoundedAndRejectsInvisibleControls', 'testLocalConfirmationPreviewDetectsAChangedServerAction', 'testLocalConfirmationPreviewRejectsMissingOrChangedRequestBinding'),
    @($arrivalChecklist, 'python .\windows-agent\trusted_unlock_admin.py pair-mobile', 'FINGERPRINT_COMPARADO', 'Comando preparado en este', 'deja `Aceptar` bloqueado'),
    @($mobileProtocol, 'python .\windows-agent\trusted_unlock_admin.py pair-mobile', 'python .\windows-agent\trusted_unlock_admin.py list-mobile', 'python .\windows-agent\trusted_unlock_admin.py revoke-mobile', 'vista local', 'no se persiste'),
